...
contract ScryptClaims is DepositsManager, IScryptChecker {
    ...
    uint256 public defaultChallengeTimeout;

    ScryptVerifier public scryptVerifier;

    event DepositBonded(uint256 claimId, address account, uint256 amount);
    event DepositUnbonded(uint256 claimId, address account, uint256 amount);
    event ClaimCreated(
        uint256 claimId,
        address claimant,
        bytes plaintext,
    ...
    event ClaimChallenged(uint256 claimId, address challenger);
    event SessionDecided(uint256 sessionId, address winner, address loser);
    ...
    event VerificationGameStarted(
        ...
        address challenger,
        uint256 sessionId
    ); //Rename to SessionStarted?
    event ClaimVerificationGamesEnded(uint256 claimId);

    struct ScryptClaim {
        address claimant;
        bytes plaintext; // the plaintext Dogecoin block header.
        bytes blockHash; // the Dogecoin blockhash.
        uint256 createdAt; // the block number at which the claim was created.
        address[] challengers; // all current challengers.
        mapping(address => uint256) sessions; //map challengers to sessionId's
        uint256 numChallengers; // is number of challengers always same as challengers.length ?
        uint256 currentChallenger; // index of next challenger to play a verification game.
        bool verificationOngoing; // is the claim waiting for results from an ongoing verificationg game.
        mapping(address => uint256) bondedDeposits; // all deposits bonded in this claim.
        bool decided;
        bool invalid;
        uint256 challengeTimeoutBlockNumber;
        bytes32 proposalId;
        IScryptCheckerListener scryptCheckerListener;
    }

    //  mapping(address => uint) public claimantClaims;
    mapping(uint256 => ScryptClaim) private claims;

    modifier onlyScryptVerifier() {
        require(
            msg.sender == address(scryptVerifier),
            "This function is restricted to the scrypt verifier contract."
        );
        _;
    }
    ...
    function initialize(ScryptVerifier initScryptVerifier) external {
        require(
            address(scryptVerifier) == address(0),
            "The contract is already initialized!"
        );
        require(
            address(initScryptVerifier) != address(0),
            "The scrypt verifier contract must be valid."
        );
    ...
    function bondDeposit(
        ...
        require(
            ...
            "Not enough balance is available. Deposit more ether."
        );
        deposits[account] -= amount;

        claim.bondedDeposits[account] = claim.bondedDeposits[account].add(
            amount
        );
    ...
    // @param account – the user's address.
    // @return – the user's deposit bonded for the claim.
    function getBondedDeposit(uint256 claimId, address account)
        public
        view
        returns (uint256)
    {
    ...
    function unbondDeposit(uint256 claimId, address account)
        public
        returns (uint256)
    {
        ScryptClaim storage claim = getValidClaim(claimId);
    ...
    function calcId(
        ...
        return
            uint256(keccak256(abi.encodePacked(claimant, _hash, _proposalId)));
    }
    ...
    function checkScrypt(
        ...
        bytes32 _hash,
        bytes32 _proposalId,
        IScryptCheckerListener _scryptCheckerListener
        ...
        require(
            deposits[_submitter] >= minDeposit,
            "The deposit of the submitter should exceed the minimum deposit."
        );

        //    uint claimId = numClaims;
        //    uint claimId = uint(keccak256(_submitter, _plaintext, _hash, numClaims));

        uint256 claimId = uint256(
            keccak256(abi.encodePacked(_submitter, _hash, _proposalId))
        );
        ...
        claim.scryptCheckerListener.scryptSubmitted(
            claim.proposalId,
            _hash,
            _data,
            msg.sender
    ...
    //
    // @param claimId – the claim ID.
    function challengeClaim(uint256 claimId) public {
        ScryptClaim storage claim = getValidClaim(claimId);

        require(!claim.decided, "The claim is already decided.");
        require(
            claim.sessions[msg.sender] == 0,
            "The sender is already challenging this claim."
        );

        require(
            deposits[msg.sender] >= minDeposit,
            "The challenger deposit should exceed the minimum deposit."
        );
        bondDeposit(claimId, msg.sender, minDeposit);

        claim.challengeTimeoutBlockNumber = block.number.add(
    ...
    // @param claimId – the claim id.
    function runNextVerificationGame(uint256 claimId) public {
        ScryptClaim storage claim = getValidClaim(claimId);
        ...
        require(
            ...
            "The claim has an ongoing verification."
        );
        ...
        if (claim.numChallengers > claim.currentChallenger) {
            ...
            uint256 sessionId = scryptVerifier.claimComputation(
                claimId,
    ...
    function sessionDecided(
        ...
        address winner,
        address loser
    ) public onlyScryptVerifier {
        ScryptClaim storage claim = getValidClaim(claimId);

        require(
            claim.verificationOngoing,
            "The claim has no verification ongoing."
        );
        claim.verificationOngoing = false;

        // reward the winner, with the loser's bonded deposit.
        uint256 depositToTransfer = claim.bondedDeposits[loser];
        claim.bondedDeposits[winner] = claim.bondedDeposits[winner].add(
            depositToTransfer
        );
    ...
    function checkClaimSuccessful(uint256 claimId) public {
        ScryptClaim storage claim = getValidClaim(claimId);
        ...
        require(
            block.number > claim.challengeTimeoutBlockNumber,
            "The claim has not exceeded the challenge timeout"
        );

        // check that all verification games have been played.
        require(
            claim.numChallengers == claim.currentChallenger,
            "Some verification games are pending."
        );
    ...
    function claimExists(ScryptClaim storage claim)
        private
        view
        returns (bool)
    {
    ...
    function firstChallenger(uint256 claimId) public view returns (address) {
        ScryptClaim storage claim = getValidClaim(claimId);
        return claim.challengers[0];
    }

    function createdAt(uint256 claimId) public view returns (uint256) {
        ScryptClaim storage claim = getValidClaim(claimId);
        return claim.createdAt;
    }

    function getSession(uint256 claimId, address challenger)
        public
        view
        returns (uint256)
    {
    ...
    function getChallengers(uint256 claimId)
        public
        view
    ...
    function getCurrentChallenger(uint256 claimId)
        public
        view
        returns (address)
    {
    ...
    function getVerificationOngoing(uint256 claimId)
        public
        view
        returns (bool)
    {
     ...
     */
    function getClaimReady(uint256 claimId) public view returns (bool) {
        ScryptClaim storage claim = claims[claimId];
        ...
        // check that the claim has exceeded the default challenge timeout.
        bool pastChallengeTimeout = block.number.sub(claim.createdAt) >
            defaultChallengeTimeout;

        // check that the claim has exceeded the claim's specific challenge timeout.
        bool pastClaimTimeout = block.number >
            claim.challengeTimeoutBlockNumber;
        ...
        return
            exists &&
            pastChallengeTimeout &&
            pastClaimTimeout &&
            noOngoingGames &&
            noPendingGames;
    }

    function getClaimStatus(uint256 claimId)
        public
        view
        returns (bool decided, bool invalid)
    {
        decided = claims[claimId].decided;
        invalid = claims[claimId].invalid;
    }

    function getValidClaim(uint256 claimId)
        private
        view
        returns (ScryptClaim storage claim)
    {
