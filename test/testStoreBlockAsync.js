...
contract('DogeRelay Async', function(accounts) {
  before(async () => {
    this.dogeRelay = await DogeRelay.new(0);
    this.scryptChecker = await ScryptCheckerDummy.new(this.dogeRelay.address, false);
    await this.dogeRelay.setScryptChecker(this.scryptChecker.address);
  });
  it("Valid async call to DogeRelay", async () => {
    ...
    await this.dogeRelay.setInitialParent(block974400Hash, 974400, 1);
    const blockScryptHash = `0x${utils.calcHeaderPoW(block974401Header)}`;
    await this.dogeRelay.storeBlockHeader(block974401Header, blockScryptHash);
    // Verify block not confirmed yet
    const bestBlockHash = await this.dogeRelay.getBestBlockHash.call();
    ...
    await this.scryptChecker.sendVerification(blockScryptHash);
    // Verify new block has been accepted
    const bestBlockHash2 = await this.dogeRelay.getBestBlockHash.call();
  ...
  it("Not valid async call to DogeRelay", async () => {
    ...
    // Send both blocks
    await this.dogeRelay.storeBlockHeader(block974402Header, block974402ScryptHash);
    await this.dogeRelay.storeBlockHeader(block974403Header, block974403ScryptHash);
    // Try to verify block second block first
    await this.scryptChecker.sendVerification(block974403ScryptHash);
    // Best block should not change
    // Out of order blocks are not accepted
    const bestBlockHash2 = await this.dogeRelay.getBestBlockHash.call();
    assert.equal(utils.formatHexUint32(bestBlockHash2.toString(16)), utils.remove0x(block974401Hash), "chain head hash is not the expected one");
    // Send first block verification
    await this.scryptChecker.sendVerification(block974402ScryptHash);
    const bestBlockHash3 = await this.dogeRelay.getBestBlockHash.call();
    ...
    await this.scryptChecker.sendVerification(block974403ScryptHash);
    const bestBlockHash4 = await this.dogeRelay.getBestBlockHash.call();
