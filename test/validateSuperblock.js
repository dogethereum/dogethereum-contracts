...
contract('validateSuperblocks', (accounts) => {
  ...
  describe('Validation superblock fields', () => {
    ...
    async function beginSuperblockChallenge() {
      ...
      result = await claimManager.queryBlockHeader(session1, hashes[1], { from: challenger });
      assert.equal(result.logs[0].event, 'QueryBlockHeader', 'Query block header');
      scryptHash = `0x${utils.calcHeaderPoW(headers[1])}`;
      result = await claimManager.respondBlockHeader(session1, scryptHash, `0x${headers[1]}`, { from: submitter });
    ...
    it('Reject invalid block bits', async () => {
      ...
      assert.equal(result.logs[1].event, 'SuperblockBattleDecided', 'Submitter failed');
      assert.equal(result.logs[2].event, 'SubmitterConvicted', 'Submitter failed');
