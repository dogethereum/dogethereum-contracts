...
contract.skip('DogeRelay2', function(accounts) {
  let dogeRelay;
  before(async () => {
    dogeRelay = await DogeRelay.deployed();
  });
  it("setInitialParent, storeBlockHeader, getBestBlockHash, getBlockHeader, getPrevBlock, getTimestamp, getBits", async () => {
    ...
    const timestamp = await dogeRelay.getTimestampPublic.call(block974401Hash);
    assert.equal(timestamp.toNumber(), 1448429048, "timestamp is not the expected one");

    const bits = await dogeRelay.getBitsPublic.call(block974401Hash);
