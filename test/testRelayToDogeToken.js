...
contract('testRelayToDogeToken', function(accounts) {
  ...
  let claimManager;
  before(async () => {
    claimManager = await DogeClaimManager.deployed();
    superblocks = await DogeSuperblocks.deployed();
  ...
  it('Relay tx to token', async () => {
    ...
    const headerAndHashes = await utils.storeSuperblockFrom974401(superblocks, claimManager, superblockSubmitterAddress);
