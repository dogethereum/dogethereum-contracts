...
contract('testDogeTokenNoOperatorOutput', function(accounts) {
  ...
  it("Accept unlock transaction without output for operator", async () => {
    const operatorKeyPair = bitcoin.ECPair.fromWIF('QSRUX7i1WVzFW6vx3i4Qj8iPPQ1tRcuPanMun8BKf8ySc8LsUuKx', DOGECOIN);
    const operator2KeyPair = bitcoin.ECPair.fromWIF('QULAK58teBn1Xi4eGo4fKea5oQDPMK4vcnmnivqzgvCPagsWHiyf', DOGECOIN);

    const operator2DogeAddress = bitcoin.payments.p2pkh({ pubkey: operator2KeyPair.publicKey, network: DOGECOIN }).address;

    const operatorPublicKeyHash = `0x${bitcoin.crypto.ripemd160(bitcoin.crypto.sha256(operatorKeyPair.publicKey)).toString('hex')}`;
    ...
    txb.addOutput(operator2DogeAddress, 1000000);
    txb.sign(0, operatorKeyPair);
